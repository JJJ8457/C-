...
void allitem() {
    ...
    // 파일로 저장
    FILE* fp_SAVE;
    if ((fp_SAVE = fopen("상품재고.txt", "w")) == NULL) {
    ...
    for (int i = 0; i < itemCount; i++) {
        ...
        fprintf(fp_SAVE, "상품명: %s\n", ITEMS[i].name);
        fprintf(fp_SAVE, "가격: %d\n", ITEMS[i].price);
        fprintf(fp_SAVE, "재고: %d\n", ITEMS[i].stock);
        fprintf(fp_SAVE, "판매량: %d\n", ITEMS[i].sold);
        fprintf(fp_SAVE, "총 판매금액: %d\n", ITEMS[i].totalSales);
        fprintf(fp_SAVE, "\n");
    }
...
int main() {
    ...
    while (1) {
        printf("원하는 메뉴를 선택하세요: 1.입고, 2.판매, 3.개별현황, 4.전체현황(파일 저장), 5.종료: ");
        scanf("%d", &menu);
        if (menu == 1) {
            itemstore();
        } else if (menu == 2) {
